...
class GetDropDownBeaconTagLibActionService extends BaseService implements ActionServiceIntf {
    ...
    Map execute(Map result) {
        ...
        try {
            List<Beacon> lstBeacon = beaconService.list()
            String html = buildDropDown(lstBeacon, attrs)
            attrs.html = html
            return attrs
        } catch (Exception e) {
            log.error(e.getMessage())
     ...
     */
    private String buildDropDown(List<Beacon> lstBeacon, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstBeacon = beaconService.listForKendoDropdown(lstBeacon, textMember, hintsText)
        }
        String jsonData = lstBeacon as JSON
