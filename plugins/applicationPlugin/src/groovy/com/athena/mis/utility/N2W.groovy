...
class N2W {
    ...
    public static String convert(BigDecimal amount) {
        try {
            ...
            if (number > 999999999999) return EMPTY_SPACE   // @todo: eliminate the limitations
            //get word of amount
    ...
    private static String processAmount(long number) {
        int n = 1;
        long word;
        String currentStr = EMPTY_SPACE
        String mainStr = EMPTY_SPACE
        while (number != 0) {
            switch (n) {
                case 1:
                    word = number % 100;
...
//                            show(STR_AND);
                        currentStr = SINGLE_SPACE + currentStr;
                    }
                    number /= 100;
                    break;
                case 2:
                    word = number % 10;
                    if (word != 0) {
                        currentStr = SINGLE_SPACE + st2[0] + SINGLE_SPACE;
                ...
                case 3:
                    word = number % 100;
                    if (word != 0) {
                        currentStr = SINGLE_SPACE + st2[1] + SINGLE_SPACE;
                        currentStr = pass(word) + currentStr;
                    }
                    number /= 100;
                    break;
                case 4:
                    word = number % 100;
                    if (word != 0) {
                        currentStr = SINGLE_SPACE + st2[2] + SINGLE_SPACE;
                        currentStr = pass(word) + currentStr;
                    }
                    number /= 100;
                    break;
                case 5:
                    word = number % 100;
                    if (word != 0) {
                        currentStr = SINGLE_SPACE + st2[3] + SINGLE_SPACE;
                        currentStr = pass(word) + currentStr;
    ...
    private static String pass(long number) {
        ...
        if (number > 19) {
            ...
            if (word == 0) {
                ...
                q = number / 10;
                passStr = st4[q - 2] + SINGLE_SPACE + st1[word];
            }
        }
