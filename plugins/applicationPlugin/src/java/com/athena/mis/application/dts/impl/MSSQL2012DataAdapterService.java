...
/**
 * Created by Mesbah on 17-Oct-15.
 */
public class MSSQL2012DataAdapterService extends DataAdapterService {
    ...
    public MSSQL2012DataAdapterService(AppDbInstance instance) {
    ...
    public String getConnectionString() {
        if (connString != null)
            return connString;
        connString = appDbInstance.getUrl() + DATABASE_NAME + appDbInstance.getDbName() + USER + appDbInstance.getUserName() + PASSWORD + appDbInstance.getPassword();
        return connString;
    }
    ...
    private Map selectCdcLog(String sql) {
        Map result = getInitResult();
        try {
            Statement statement = connection.createStatement();
            ResultSet rs = statement.executeQuery(sql);
