 ...
 */
class ListAnnouncementAppMailActionService extends BaseService implements ActionServiceIntf {
    ...
    public Map execute(Map result) {
        try {
            ...
            Closure additionalParam = {
                ilike("recipients", '%'+ "${appUser.loginId}" +'%')
                and {
                    eq('isAnnouncement', true)
                    eq('hasSend', true)
                }
            }
            Map resultMap = super.getSearchResult(result, ListAnnouncementAppMailActionServiceModel.class, additionalParam)
