...
class DeleteSystemEntityActionService extends BaseService implements ActionServiceIntf {
    ...
    private static
    final String HAS_ASSOCIATION_PURCHASE_ORDER = " purchase order(s) associated with selected system entity"
    ...
    private static
    final String HAS_ASSOCIATION_FXD_ASSET_DETAILS = " fixed asset details(s) associated with selected system entity"
    private static final String HAS_ASSOCIATION_VOUCHER = " voucher(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_VOUCHER_TYPE_COA = " voucher type coa(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_ATTACHMENT = " attachment(s) associated with selected system entity"
    private static final String HAS_ASSOCIATION_APP_NOTE = " note(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_CONTENT_CATEGORY = " content category(s) associated with selected system entity"
    ...
    private static
    final String HAS_ASSOCIATION_PRODUCTION_DETAILS = " production details(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_INVENTORY_TRANSACTION = " inventory transaction(s) associated with selected system entity"
    private static final String HAS_ASSOCIATION_INVENTORY = " inventory(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_USER_ENTITY = " user entity mapping(s) associated with selected system entity"
    private static final String HAS_ASSOCIATION_EXH_TASK = " exh task(s) associated with selected system entity"
    private static final String HAS_ASSOCIATION_DOC_QUESTION = " question(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_DOC_DOCUMENT_INDEX_TYPE = " document index type(s) associated with selected system entity"
    private static final String HAS_ASSOCIATION_DOC_DOCUMENT = " document(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_DOC_DOCUMENT_TRASH = " document trash(es) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_DOC_DOCUMENT_CATEGORY_TYPE = " document category type(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_DPL_OFFLINE_DATA_FEED_STATUS = " offline data feed file(s) associated with selected system entity"
    private static
    ...
    private static final String HAS_ASSOCIATION_EXH_CUSTOMER = " exh customer(s) associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_EXH_CUSTOMER_TRACE = " exh customer trace(s) associated with selected system entity"
    ...
    private static
    final String HAS_ASSOCIATION_EL_COURSE = " course(s) are associated with selected system entity"
    private static
    final String HAS_ASSOCIATION_EL_ASSIGNMENT = " assignment(s) are associated with selected system entity"

    /**
     * 1. Pull system entity object
     * 2. Check the existence of system entity object
     * 3. Check the selected system entity object is Reserved or not
     * 4. Check association with other domains
    ...
    public Map executePreCondition(Map params) {
        try {
            ...
            if (systemEntity.reservedId > 0) {
                return super.setError(params, IS_RESERVED_MSG)
            }

            long companyId = super.getCompanyId()
            String msg = hasAssociation(systemEntity, companyId)
    ...
    /**
     * 1. Delete system entity from db
    ...
    public Map execute(Map result) {
        try {
            ...
            updateSystemEntityCacheService(systemEntity)
            return result
        } catch (Exception ex) {
            log.error(ex.getMessage())
            throw new RuntimeException(ex)
    ...
    public Map buildSuccessResultForUI(Map result) {
        return super.setSuccess(result, SYS_ENTITY_DELETE_SUCCESS_MSG)
    }
    ...
    private String hasAssociation(SystemEntity systemEntity, long companyId) {
        ...
        switch (systemEntityTypeId) {
            ...
            case appSystemEntityCacheService.SYS_ENTITY_TYPE_CONTENT_ENTITY:
                ...
                if (count > 0) {
                    return count.toString() + HAS_ASSOCIATION_ATTACHMENT
                }
                break
            ...
            case ptProjectTrackImplService?.getSystemEntityTypeChangeRequestStatus():
                ...
                break
            case appSystemEntityCacheService.SYS_ENTITY_TYPE_GENDER:
                if (PluginConnector.isPluginInstalled(ExchangeHousePluginConnector.PLUGIN_NAME)) {
                    count = countExhCustomerForGender(systemEntityId, companyId)
                    if (count > 0) {
                        return count.toString() + HAS_ASSOCIATION_EXH_CUSTOMER
                    }

                    count = countExhCustomerTraceForGender(systemEntityId, companyId)
                    if (count > 0) {
                        return count.toString() + HAS_ASSOCIATION_EXH_CUSTOMER_TRACE
                    }
                }
                break
            ...
            case documentImplService?.getSystemEntityTypeDocumentIndex():
                if (PluginConnector.isPluginInstalled(DocumentPluginConnector.PLUGIN_NAME)) {
                    count = countDocDocumentIndexType(systemEntityId, companyId)
                    if (count > 0) {
    ...
    private static final String COUNT_EXH_CUSTOMER_FOR_GENDER = """
            SELECT COUNT(id) AS count
            FROM exh_customer
            WHERE gender_id = :systemEntityId AND
                  company_id = :companyId
    """

    /**
     * get count of exh customer for gender
     * @param systemEntityId
     * @param companyId
     * @return - count of exh customer
     */
    private int countExhCustomerForGender(long systemEntityId, long companyId) {
        Map queryParams = [
                systemEntityId: systemEntityId,
                companyId     : companyId
        ]
        List results = executeSelectSql(COUNT_EXH_CUSTOMER_FOR_GENDER, queryParams)
        int count = results[0].count
        return count
    }

    private static final String COUNT_EXH_CUSTOMER_TRACE_FOR_GENDER = """
            SELECT COUNT(id) AS count
     ...
     * @return - count of exh customer
     */
    private int countExhCustomerTraceForGender(long systemEntityId, long companyId) {
        Map queryParams = [
                systemEntityId: systemEntityId,
                companyId     : companyId
        ]
        List results = executeSelectSql(COUNT_EXH_CUSTOMER_TRACE_FOR_GENDER, queryParams)
        int count = results[0].count
        return count
    }
    ...
    private static final String COUNT_DOC_DOCUMENT_INDEX_TYPE = """
            SELECT COUNT(id) AS count
     ...
     */
    private int countDocDocumentIndexType(long systemEntityId, long companyId) {
        Map queryParams = [systemEntityId: systemEntityId, companyId: companyId]
        List results = executeSelectSql(COUNT_DOC_DOCUMENT_INDEX_TYPE, queryParams)
        int count = results[0].count
        return count
    }
    ...
    /**
     * get count of DocOfflineDataFeedFile
     * @param systemEntityId
     * @param companyId
     * @return - count of DocOfflineDataFeedFile
     */
