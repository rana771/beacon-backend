...
class GetDropDownCountryTagLibActionService extends BaseService implements ActionServiceIntf {
    ...
    Map execute(Map result) {
        ...
        try {
            List<AppCountry> lstCountry = appCountryService.list()
            String html = buildDropDown(lstCountry, attrs)
            attrs.html = html
            return attrs
        } catch (Exception e) {
            log.error(e.getMessage())
     ...
     */
    private String buildDropDown(List<AppCountry> lstCountry, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstCountry = appCountryService.listForKendoDropdown(lstCountry, textMember, hintsText)
        }
        String jsonData = lstCountry as JSON
