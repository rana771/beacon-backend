...
/**
 *  Action Service for Maintenance Script Scheduler
 *  For details go through Use-Case doc named 'AppMaintenanceScriptJobActionService'
 */
class AppMaintenanceShellScriptJobActionService extends BaseService implements ActionServiceIntf {

    private Logger log = Logger.getLogger(getClass())

    private static final String OBJECT_NOT_FOUND = "Maintenance shell script not found"
    private static final String SERVER_INSTANCE_NOT_FOUND = "Native server instance not found"
    private static
    final String ERR_MSG = "Error occurred in AppMaintenanceScriptJobActionService. Check log for details..."
    private static final String TRANSACTION_CODE = "AppMaintenanceShellScriptJobActionService"
    private static
    final String MAIL_TEMPLATE_NOT_FOUND = "Maintenance Script mail not send due to absence of mail template"
    private static final String SUCCESS_MESSAGE = "Maintenance Script has been executed successfully."
    private static
    final String NOT_PRODUCTION_MODE = "Maintenance shell script mail is not sent because application is not in production mode"
    ...
    public Map execute(Map parameters) {
        ...
        try {
            AppShellScript shellScript = appShellScriptService.findByNameAndCompanyId(AppShellScript.MAINTENANCE_SCRIPT, companyId)
            if (!shellScript) {
