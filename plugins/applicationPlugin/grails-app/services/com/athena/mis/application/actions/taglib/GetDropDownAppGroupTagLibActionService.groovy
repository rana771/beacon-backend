...
/*Renders html of 'select' for AppGroup objects */

class GetDropDownAppGroupTagLibActionService extends BaseService implements ActionServiceIntf {
    ...
    public Map execute(Map result) {
        try {
            List<AppGroup> lstAppGroup = appGroupService.list()
            String html = buildDropDown(lstAppGroup, result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @return - html string for select
     */
    private String buildDropDown(List<AppGroup> lstAppGroup, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstAppGroup = appGroupService.listForKendoDropdown(lstAppGroup, null, hintsText)
            // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstAppGroup as JSON
