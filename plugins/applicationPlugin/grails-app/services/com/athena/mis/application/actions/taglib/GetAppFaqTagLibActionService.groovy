 ...
 * for details go through use-case "GetAppFaqTagLibActionService"
 */
class GetAppFaqTagLibActionService extends BaseService implements ActionServiceIntf {
    ...
    private static final String DEFAULT_TEMPLATE = "<span>#=htmlDecode(question)#</span><br><blockquote><span>#=htmlDecode(answer)#</span><footer>By #:createdBy# On #:createdOn#</footer></blockquote>"
    ...
    public Map execute(Map result) {
        try {
            String html = buildAppFaqHtml(result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @return - html
     */
    private String buildAppFaqHtml(Map attrs) {
        ...
        String divTitle = title ? "<div id='listViewTitlePanel' class='panel panel-primary'><div class='panel-heading'><div class='panel-title'>${title}</div></div><div class='panel-body'> " : EMPTY_SPACE
        ...
        String strDataSource = """
            new kendo.data.DataSource({
                transport: {
                    read: {
                        url: "/appFaq/listFaq?entityTypeId=${entityTypeId}&entityId=${entityId}&sortOrder=${sortOrder}",
                        dataType: "json",
                        type: "post"
                    }
                },
        ...
        <script type='text/javascript'>
            \$(document).ready(function() {
                ...
                \$('#pagerFaq').kendoPager({dataSource: ${dataSource}, refresh: true});
