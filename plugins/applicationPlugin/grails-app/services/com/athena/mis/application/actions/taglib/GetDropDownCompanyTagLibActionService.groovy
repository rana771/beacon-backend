...
/* Renders html of 'select' for Company objects */

class GetDropDownCompanyTagLibActionService extends BaseService implements ActionServiceIntf {
    ...
    public Map execute(Map result) {
        try {
            List<Company> lstCompany = companyService.list()
            String html = buildDropDown(lstCompany, result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @param lstCompany - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<Company> lstCompany, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstCompany = companyService.listForKendoDropdown(lstCompany, null, hintsText)
            // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstCompany as JSON
