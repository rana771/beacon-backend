...
import com.athena.mis.application.entity.Sampling100Star

class Sampling100StarService extends BaseDomainService {

    @Override
    public void init() {
        domainClass = Sampling100Star.class
    }
    ...
    private boolean field96 = true
    private boolean field97 = true
    private boolean field98

    public String getSampling100StarSql(BenchmarkStar benchmark, long seed, List<Long> lstSmallIds, List<Long> lstMediumIds) {
        long limit = benchmark.recordPerBatch + seed
        StringBuffer sql = new StringBuffer("""INSERT INTO sampling100star (field1, field2, field3, field4, field5, field6, field7, field8, field9, field10,
        ...
        for (long i = seed; i < limit; i++) {
            ...
            String s = """('${field1 + count}', '${field2 + count}', '${field3 + count}', '${field4 + count}', '${field5 + count}', '${field6 + count}', '${field7 + count}', '${field8 + count}', '${field9 + count}', '${field10 + count}',
                   ...
                   '${field91 + i}', '${field92 + i}', '${field93 + i}', '${field94 + i}', ${field95}, ${field96}, ${field97}, ${field98}, '${lstSmallIds[index]}', '${lstMediumIds[index]}'),"""
            sql.append(s)
            j++
            index++
        }
    ...
    public void create(String sql) {
        ...
        if (insertResult.size() <= 0) {
            throw new RuntimeException('Error occurred while insert sampling100fStar information')
        }
    }
