...
/*Renders html of 'select' for Vehicle objects */
class GetDropDownVehicleTagLibActionService extends BaseService implements ActionServiceIntf {
    ...
    public Map execute(Map result) {
        try {
            List<Vehicle> lstVehicle = vehicleService.list()
            String html = buildDropDown(lstVehicle, result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @param lstVehicle - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<Vehicle> lstVehicle, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstVehicle = vehicleService.listForKendoDropdown(lstVehicle, null, hintsText)
            // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstVehicle as JSON
