package com.athena.mis.application.actions.report.benchmark
...
class DownloadBenchmarkReportActionService extends BaseService implements ActionServiceIntf {
    ...
    public Map executePreCondition(Map params) {
        try {
            ...
            Benchmark benchmark = (Benchmark) benchmarkService.read(benchmarkId)
            if (!benchmark) {
    ...
    public Map execute(Map result) {
        try {
            Benchmark benchmark = (Benchmark) result.get(BENCHMARK_OBJ)
     ...
     */
    private Map getBenchmarkReport(Benchmark benchmark, long entityTypeId) {
        ...
        SystemEntity benchmarkType = (SystemEntity) appSystemEntityCacheService.read(benchmark.volumeTypeId, appSystemEntityCacheService.SYS_ENTITY_TYPE_BENCHMARK, benchmark.companyId)
        ...
        reportParams.put(VOLUME_TYPE_NAME, benchmarkType.key)
        reportParams.put(TOTAL_RECORD, benchmark.totalRecord)
        reportParams.put(RECORD_PER_BATCH, benchmark.recordPerBatch)
    ...
    private GroovyRowResult getExecutionTime(long benchmarkId, long entityTypeId) {
        Map queryParams = [benchmarkId: benchmarkId, transactionLogTypeId: entityTypeId]
