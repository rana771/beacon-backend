...
/*Renders html of 'select' for Supplier objects */
class GetDropDownSupplierTagLibActionService extends BaseService implements ActionServiceIntf {
    ...
    public Map execute(Map result) {
        try {
            List<Supplier> lstSupplier = supplierService.list()
            String html = buildDropDown(lstSupplier, result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @param lstSupplier - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<Supplier> lstSupplier, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstSupplier = supplierService.listForKendoDropdown(lstSupplier, null, hintsText)
            // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstSupplier as JSON
