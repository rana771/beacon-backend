...
class GetDropDownItemTypeTagLibActionService extends BaseService implements ActionServiceIntf {
    ...
    public Map executePreCondition(Map params) {
        try {
            ...
            String strCategoryId = params.get(CATEGORY_ID)
            if (strCategoryId) {
                long systemEntityId = Long.parseLong(strCategoryId)
                params.put(CATEGORY_ID, new Long(systemEntityId)) // set the categoryId
            } else {
    ...
    public Map execute(Map result) {
        try {
            ...
            if (result.get(CATEGORY_ID)) {
                systemEntityId = (Long) result.get(CATEGORY_ID)
                SystemEntity itemSysEntityObject = appSystemEntityCacheService.readByReservedId(systemEntityId, appSystemEntityCacheService.SYS_ENTITY_TYPE_ITEM_CATEGORY, super.getCompanyId())
                lstItemType = listItemType(itemSysEntityObject.id)
            } else {
                lstItemType = listItemType(systemEntityId)
            }
            String html = buildDropDown(lstItemType, result)
            result.html = html
            return result
        } catch (Exception e) {
            log.error(e.message)
    ...
    private List<ItemType> listItemType(long systemEntityId) {
        List<ItemType> lstItemType
        if (systemEntityId > 0) {
            lstItemType = itemTypeService.listByCategoryId(systemEntityId)
        } else {
            lstItemType = itemTypeService.list()
        }
        return lstItemType
    }
    ...
    private String buildDropDown(List<ItemType> lstItemType, Map dropDownAttributes) {
        ...
        if (showHints.booleanValue()) {
            lstItemType = itemTypeService.listForKendoDropdown(lstItemType, null, hintsText)
