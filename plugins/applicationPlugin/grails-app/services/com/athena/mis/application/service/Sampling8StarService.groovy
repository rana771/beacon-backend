...
import com.athena.mis.application.entity.Sampling8Star

class Sampling8StarService extends BaseDomainService {

    @Override
    public void init() {
        domainClass = Sampling8Star.class
    }
    ...
    public Map getSampling8StarSql(BenchmarkStar benchmark, long seed) {
        long limit = benchmark.recordPerBatch + seed
        StringBuffer sql = new StringBuffer("INSERT INTO sampling8star (field1, field2, field3, field4, field5, field6, field7, field8) VALUES ")
        List<Long> lstSmallIds = []
        long j = 0L
        for (long i = seed; i < limit; i++) {
            String count = i.toString()
            Long idCol = field6 + j + seed
            String s = "('${field1 + count}', '${field2 + count}', '${field3 + count}', '${field4 + count}', '${field5 + count}', '${idCol.longValue()}', '${field7 + i}', '${field8 + i}'),"
            sql.append(s)
            lstSmallIds << idCol
            j++
        }
        return [strSql: sql.substring(0, sql.length() - 1), lstIds: lstSmallIds]
    }

    public void create(String sql) {
        List insertResult = executeInsertSql(sql)
        if (insertResult.size() <= 0) {
            throw new RuntimeException('Error occurred while insert sampling8fStar information')
        }
    }
