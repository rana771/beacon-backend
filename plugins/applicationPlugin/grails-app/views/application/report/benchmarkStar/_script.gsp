<script language="javascript">
    ...
    function downloadBenchmarkReport() {
        ...
        if (confirm('Do you want to download the benchmark report now?')) {
            var url = "${createLink(controller:'benchmarkStar', action: 'downloadBenchmarkReport')}" + params;
    ...
    function createChart() {
        $("#chart").kendoChart({
            title: {
                text: "BenchmarkStar Transaction Status \n ${benchmark?.name}"
            },
            legend: {
                position: "bottom"
            },
            ...
            categoryAxis: {
                title: {
                    text: "Transaction Span (${benchmark?.recordPerBatch*3} per batch)"
                },
