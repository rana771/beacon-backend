...
<ul id="menuGrid" class="kendoGridMenu">
    <app:ifAllUrl urls="/benchmarkStar/update">
        <li onclick="editBenchmarkStar();"><i class="fa fa-edit"></i>Edit</li>
    </app:ifAllUrl>
    <app:ifAllUrl urls="/benchmarkStar/delete">
        <li onclick="deleteBenchmark();"><i class="fa fa-trash-o"></i>Delete</li>
    </app:ifAllUrl>
    <app:ifAllUrl urls="/benchmarkStar/execute">
        <li onclick="executeBenchmark();"><i class="fa fa-play"></i>Execute</li>
    </app:ifAllUrl>
    <app:ifAllUrl urls="/benchmarkStar/stopBenchMark">
        <li onclick="stopBenchmark();"><i class="fa fa-stop"></i>Stop</li>
    </app:ifAllUrl>
    <app:ifAllUrl urls="/benchmarkStar/clear">
    ...
    <app:ifAllUrl urls="/benchmarkStar/showReport">
...
<script language="javascript">
    var benchMarkStarGrid, benchmarkStarDataSource, benchmarkStarModel, totalRecord, recordPerBatch, dropDownDbInstance,
            entityTypeId;
    ...
    function onLoadBenchmarkPage() {
        ...
        initializeForm($("#benchmarkStarForm"), onSubmitBenchmark);
        ...
        // update page title
        $(document).attr('title', "MIS - Create BenchmarkStar");
        loadNumberedMenu(MENU_ID_APPLICATION, "#benchmarkStar/show");
    }

    function executePreCondition() {
        if (!validateForm($("#benchmarkStarForm"))) {   // check kendo validation
            return false;
        }
    ...
    function onSubmitBenchmark() {
        ...
        var actionUrl = null;
        if ($('#id').val().isEmpty()) {
            actionUrl = "${createLink(controller: 'benchmarkStar', action: 'create')}";
        } else {
            var type = 'update';
            if (isApplicableToPerform(type) == false) {
                return;
            }
            actionUrl = "${createLink(controller: 'benchmarkStar', action: 'update')}";
        }
        ...
        jQuery.ajax({
            type: 'post',
            data: jQuery("#benchmarkStarForm").serialize(),
            url: actionUrl,
        ...
        if (result.isError) {
            ...
            try {
                ...
                if ($('#id').val().isEmpty() && newEntry != null) { // newly created
                    var gridData = benchMarkStarGrid.dataSource.data();
    ...
    function updateSelectedRow(newEntry) {
        var selectedRow = benchMarkStarGrid.select();
        var allItems = benchMarkStarGrid.items();
        var selectedIndex = allItems.index(selectedRow);
        benchMarkStarGrid.removeRow(selectedRow);
        benchMarkStarGrid.dataSource.insert(selectedIndex, newEntry);
    }

    function resetForm() {
        // clear all errors, validation messages & form values and bind onFocus method
        clearForm($("#benchmarkStarForm"), $('#name'));
        initObservable();
    ...
    function deleteBenchmark() {
        ...
        showLoadingSpinner(true);
        var benchmarkId = getSelectedIdFromGridKendo(benchMarkStarGrid);

        $.ajax({
            url: "${createLink(controller: 'benchmarkStar', action: 'delete')}?id=" + benchmarkId,
    ...
    function executePreConditionForDelete() {
        if (executeCommonPreConditionForSelectKendo(benchMarkStarGrid, 'benchmarkStar') == false) {
            return false;
        }
    ...
    function executePostConditionForDelete(data) {
        ...
        var row = benchMarkStarGrid.select();
        row.each(function () {
            benchMarkStarGrid.removeRow($(this));
        });
        resetForm();
        showSuccess(data.message);
    }

    function editBenchmarkStar() {
        if (executeCommonPreConditionForSelectKendo(benchMarkStarGrid, 'benchmarkStar') == false) {
            return;
        }
        showCreatePanel($('div.expand-div'), $('#gridBenchmarkStar'));
        var benchmarkObj = getSelectedObjectFromGridKendo(benchMarkStarGrid);
        showBenchmark(benchmarkObj);
    }

    function showBenchmark(benchmarkObj) {
        benchmarkStarModel.set('benchmarkStar', benchmarkObj);
    ...
    function executeBenchmark() {
        ...
        var benchmarkId = getSelectedIdFromGridKendo(benchMarkStarGrid);
        $.ajax({
            url: "${createLink(controller: 'benchmarkStar', action: 'execute')}?id=" + benchmarkId,
    ...
    function stopBenchmark() {
        ...
        var benchmarkId = getSelectedIdFromGridKendo(benchMarkStarGrid);
        $.ajax({
            url: "${createLink(controller: 'benchmarkStar', action: 'stopBenchMark')}?id=" + benchmarkId,
    ...
    function clearBenchmark() {
        ...
        var benchmarkId = getSelectedIdFromGridKendo(benchMarkStarGrid);
        $.ajax({
            url: "${createLink(controller: 'benchmarkStar', action: 'clear')}?id=" + benchmarkId,
            success: executePostConditionForExecute,
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                afterAjaxError(XMLHttpRequest, textStatus)
            },
    ...
    function monitorBenchmarkTransaction() {
        ...
        var benchmarkId = getSelectedIdFromGridKendo(benchMarkStarGrid);
        var loc = "${createLink(controller:'transactionLog', action: 'show')}?oId=" + benchmarkId + "&url=benchmarkStar/show" + "&entityTypeId=" + entityTypeId;
    ...
    function showBenchmarkReport() {
        ...
        var benchmarkId = getSelectedIdFromGridKendo(benchMarkStarGrid);
        var params = "?oId=" + benchmarkId + "&url=benchmarkStar/show";
        var loc = "${createLink(controller:'benchmarkStar', action: 'showReport')}" + params;
        router.navigate(formatLink(loc));
        return false;
    }

    function isApplicableToPerform(type) {
        var startTime = getSelectedValueFromGridKendo(benchMarkStarGrid, 'startTime');
        var endTime = getSelectedValueFromGridKendo(benchMarkStarGrid, 'endTime');
    ...
    function initDataSource() {
        benchmarkStarDataSource = new kendo.data.DataSource({
            transport: {
                read: {
                    url: "/benchmarkStar/list",
                    dataType: "json",
                    type: "post"
                }
            },
    ...
    function initBenchmarkGrid() {
        initDataSource();
        $("#gridBenchmarkStar").kendoGrid({
            dataSource: benchmarkStarDataSource,
            ...
            columns: [
                ...
                {field: "recordPerBatch", title: "Record Per Batch", width: 40, sortable: false, filterable: false},
                {
                    ...
                    template: "#= startTime ? kendo.toString(kendo.parseDate(startTime, 'yyyy-MM-ddTHH:mm:ss'), 'dd-MMM-yyyy [hh:mm:ss tt]'):'0:0' #"
                },
                {
                    field: "endTime", title: "End Time", width: 60, sortable: false, filterable: false,
                    template: "#= endTime ? kendo.toString(kendo.parseDate(endTime, 'yyyy-MM-ddTHH:mm:ss'), 'dd-MMM-yyyy [hh:mm:ss tt]'):'0:0' #"
                },
                {
                    title: "Duration", width: 30, sortable: false, filterable: false,
                    template: "#=  (startTime && endTime) ? msToTime(endTime-startTime) : '00:00:00.0' #"
                },
                {
        ...
        benchMarkStarGrid = $("#gridBenchmarkStar").data("kendoGrid");
        $("#menuGrid").kendoMenu();
    }

    function reloadKendoGrid() {
        benchMarkStarGrid.dataSource.filter([]);
    }
    ...
    function initObservable() {
        benchmarkStarModel = kendo.observable(
                {
                    benchmarkStar: {
                        id: "",
                        version: "",
        ...
        kendo.bind($("#application_top_panel"), benchmarkStarModel);
    }
